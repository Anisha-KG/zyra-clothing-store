...
const shopPage = async (req, res) => {
  try {
    ...
    /* =========================================================
       ✅ UPDATE bestOffer & finalPrice IN DATABASE
       ========================================================= */

    const productsToUpdate = await Product.find({ isBlocked: false });
    ...
    // Build query properly (apply collation only for name sorting)
    let productQueryBuilder = Product.find(productQuery).sort(sortQuery);

    if (sortBy === 'az' || sortBy === 'za') {
      productQueryBuilder = productQueryBuilder.collation({
        locale: 'en',
        strength: 2
      });
    }
    ...
    const variants = filteredVariants.filter(v =>
      productIds.includes(v.product.toString())
    );
